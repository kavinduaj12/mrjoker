...
def is_user_admin(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    if (
        ...
        or user_id in [1541292599, 948247711]
...
def sudo_plus(func):
    ...
    def is_sudo_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user and is_sudo_plus(chat, user.id):
            ...
            update.effective_message.reply_text(
                "Who non-admin telling me what to do? You want a Gali?"
            )
...
def user_admin(func):
    ...
    def is_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user and is_user_admin(chat, user.id):
            ...
            update.effective_message.reply_text(
                "Who dis non-admin telling me what to do? You want a Abuse word?"
            )
...
def bot_admin(func):
    ...
    def is_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            not_admin = "I'm not admin! - please promote me with all rights"
        else:
...
def user_can_ban(func):
    ...
    def user_is_banhammer(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if (
            ...
            and user not in [1541292599, 948247711]
        ):
            update.effective_message.reply_text(
                "Sorry son, He is your stop it."
            )
            return ""
        return func(update, context, *args, **kwargs)
